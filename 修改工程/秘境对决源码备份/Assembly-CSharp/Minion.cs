...
public class Minion : Character
{
	...
	public override IEnumerator Attack(Character target)
	{
		...
		if (minionPreAttackEvent.Status != PreStatus.Cancelled)
		{
			...
			AttackedEvent value = new AttackedEvent
			{
				Damage = previousTargetHealth - target.CurrentHealth,
				Target = target
			};
			yield return this.Mechanics.OnAttacked.Fire(value);
		}
	...
	{
		return this.IsFriendlyOf(other) && this.IsAlive() && other.IsAlive() && Math.Abs(this.GetPosition() - other.GetPosition()) == 1;
	}

	public bool IsDeadNextTo(Minion other)
	{
		return this.IsFriendlyOf(other) && Math.Abs(this.GetPosition() - other.GetPosition()) == 1;
	}
