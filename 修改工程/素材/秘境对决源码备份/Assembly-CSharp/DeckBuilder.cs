...
public class DeckBuilder : MonoBehaviour
{
	...
	{
		this.CurrentDeck = new SavedDeck("Normal Deck", heroClass);
		MenuManager.Instance.DeckBuilderNameField.text = "Normal Deck";
	}
	...
	{
		int num = 0;
		using (List<BuilderSelectedCardController>.Enumerator enumerator = this.SelectedCards.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				if (enumerator.Current.Type == SelectedCardType.Double)
				{
					num++;
				}
				num++;
	...
	{
		foreach (SavedCard savedCard in savedDeck.SavedCards)
		{
			this.AddController(savedCard);
			yield return null;
		}
		List<SavedCard>.Enumerator enumerator = default(List<SavedCard>.Enumerator);
		this.UpdateAll();
		yield break;
		yield break;
	}
	...
	public void EnlargeOn(HeroClass heroClass)
	{
		foreach (BuilderClassButton builderClassButton in UnityEngine.Object.FindObjectsOfType<BuilderClassButton>())
		{
			...
			{
				builderClassButton.transform.localScale = Vector3.one * 1.3f;
			}
			else
			{
